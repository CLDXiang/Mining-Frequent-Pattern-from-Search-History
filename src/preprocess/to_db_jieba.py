...
def file_to_db(input_file_path, output_file_path):
    ...
    with open(input_file_path, 'r', encoding='utf-8') as f:
        while True:
            ...
            # 切分关键词：第二种策略：通过加号和空白符分开后，再用 jieba 搜索引擎模式分词
            keywords_temp = ' '.join([kw.strip() for kw in re.split(r'[+\s]', keywords) if kw.strip() != ''])

            keywords = jieba.analyse.extract_tags(keywords_temp)
...
def to_db_all():
    log_print = def_log_print('to_db_jieba')
    data_dir = '../../data/'
    count_total = 0
    ...
    for file in files:
        ...
        count = file_to_db(path.join(data_dir, 'clean', file),
                                                    path.join(data_dir, 'DB_jieba', file)[
                                                    :-len('.txt')]+'_DB_jieba.txt')
